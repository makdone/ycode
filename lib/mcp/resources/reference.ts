...
const DESIGN_REFERENCE_JSON = JSON.stringify({
  ...
  categories: {
    layout: {
      ...
      flex: { type: 'enum', values: ['1', 'auto', 'initial', 'none'], description: 'Flex child shorthand. "1" makes the child fill the remaining space of its flex parent.' },
      flexGrow: { type: 'enum', values: ['1', '0'], description: 'Flex child: allow (1) or prevent (0) growing.' },
      flexShrink: { type: 'enum', values: ['1', '0'], description: 'Flex child: allow (1) or prevent (0) shrinking.' },
      order: { type: 'string', examples: ['first', 'last', 'none', '1', '2'], description: 'Flex child visual order (first | last | none | 1..12).' },
